...
public class BlogPostDAO {
    ...
    public void addPostComment(final String name, final String email, final String body, final String permalink) {
        ...
        BasicDBObject match = new BasicDBObject("permalink", permalink);
        if (match != null) {
            BasicDBObject commentTobeAdded = new BasicDBObject().append( "author", name).append("body", body);
            if (StringUtils.isNotEmpty(email)) {
                commentTobeAdded.put("email", email);
            }

            BasicDBObject update = new BasicDBObject();
            update.put("$push", new BasicDBObject("comments", commentTobeAdded));

            postsCollection.update(match, update);
        }
    }
