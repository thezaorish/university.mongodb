...
public class UserDAO {
    ...
    public boolean addUser(String username, String password, String email) {
        ...
        DBObject userToBeInserted = new BasicDBObject("_id", username).append("password", passwordHash);

        if (email != null && !email.equals("")) {
            // XXX WORK HERE
            // if there is an email address specified, add it to the document too.
            userToBeInserted.put("email", email);
        }

        try {
            // XXX WORK HERE
            // insert the document into the user collection here
            usersCollection.insert(userToBeInserted);
            return true;
